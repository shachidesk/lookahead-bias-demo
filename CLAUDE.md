...
**合否は、数字を組み立てる前に出す。** `run_backtest` は結果を返す直前ではなく
**返り値を作る前**に `guards.require_pass()` を呼び、`lookahead_guards: PASS (...)` の
行を出す。PASS でなければそこで送出して数字を返さない。良い数字を見てから検証すると、
確かめる動機がそこで一番弱くなる。

**歯止めを足したら `GuardLog` の `expected` にも足す。** 評価回数を数えていない歯止めは、
踏まれなくても PASS が出てしまう。**「作動しなかった」は正常だが「評価されなかった」は
異常**で、1つでも 0 なら判定は `VACUOUS`(`src/lookahead_demo/guardlog.py`)。
`tests/test_guard_log.py` が検査している。
